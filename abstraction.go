...
start := time.Now()
mode := "monolith"
if AbsCtx.Microservice { mode = "microservice" }
		returnedResp, err := (AbsCtx.dispatchTable[verb].fn.(func(*commondata.ReqCtx, *Req) (*Resp, error))(ctx, req))
f, _ := os.OpenFile(fmt.Sprintf("%s_latency.log", verb), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
defer f.Close()
fmt.Fprintf(f, "Latency for %s (%s): %v\n", verb, mode, time.Since(start))
		if err != nil {
			return nil, err
		} else {
